...
const TimelineItem: React.FC<TimelineItemProps> = ({ entry, isActive, onClick }) => (
  <div
    onClick={onClick}
    className={`
      relative cursor-pointer w-full transition-colors duration-200 rounded-base
      pl-16 pr-4 py-4
      border-l-4 border-transparent       /* reserve 4px for the spine always */
      ${
        isActive
          ? 'bg-teal-100 dark:bg-gray-700' /* only background changes when active */
          : 'hover:bg-gray-100 dark:hover:bg-gray-600'
      }
    `}
  >
    {/* Minimalistic pointer only - vertical line is now in the container */}
    <div className="absolute left-6 top-1/2 -translate-y-1/2 w-2 h-2 bg-black dark:bg-white rounded-base z-10"></div>
  ...
  <div className="relative">
    ...
    <div
      className="absolute bottom-2 left-1/2 -translate-x-1/2 z-50
                    ...
                    p-4 rounded-base shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]
                    dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.3)]
                    min-w-[250px] max-w-[350px]"
    >
      <h2
        className="font-black text-xl mb-2 border-b-4 border-border dark:border-darkBorder pb-2
    ...
    <div
      className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4
                    ...
                    border-border dark:border-darkBorder rotate-45 z-40" // Ensure triangle is below box but above map elements potentially
    ></div>
  </div>
);
...
const TimelineContainer: React.FC<TimelineContainerProps> = ({ isOpen, onClose, children }) => (
  <div
    className={`
        ...
        border-r-4 border-border dark:border-darkBorder
  ...
  >
    <div className="flex-none flex items-center justify-between p-4 border-b-4 border-border dark:border-darkBorder bg-white dark:bg-darkBg">
    ...
    {[
      ...
      <button
        ...
        className="bg-bg dark:bg-darkBg text-text dark:text-darkText w-12 h-12 text-2xl font-black
                         border-4 border-border dark:border-darkBorder rounded-base shadow-shadow
  ...
  return (
    ...
    <div className="relative p-4 sm:p-6 md:p-8 bg-white dark:bg-black py-16">
      ...
      <div className="max-w-full mx-auto px-5 relative z-10">
        {/* Title Section */}
        <div
          className="w-full bg-bg border-4 border-border dark:border-darkBorder dark:bg-darkBg rounded-base
                            shadow-shadow
        ...
        <div
          className="relative border-4 border-border dark:border-darkBorder bg-white dark:bg-darkBg
                            shadow-shadow
                            h-[550px] md:h-600px lg:h-[700px] xl:h-[750px] // Improved responsive height
                            overflow-hidden rounded-base" // Added slight rounding
        >
          {/* Map Container */}
          <div ref={mapRef} className="w-full h-full" aria-label="Interactive Journey Map">
            ...
            <div
              className="absolute top-4 right-4 left-auto md:right-20 lg:right-24 z-10
                            bg-white/80 dark:bg-black/80 backdrop-blur-sm p-2 px-3 rounded-base
          ...
          {isMobile && !isTimelineOpen && (
            <button
              ...
              className="absolute top-4 left-4 z-30 p-3 bg-bg dark:bg-darkBg text-black dark:text-darkText
                                 border-4 border-black dark:border-white rounded-base
                                 shadow-shadow
                                 hover:shadow-none hover:translate-x-1 hover:translate-y-1
                                 transition-all duration-200"
            >
